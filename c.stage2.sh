...
controller="cloud"
my_ip=$(hostname)
NOVA_DBPASS=$(cat openstack_passwords.txt | grep dbnova_pass | awk '{print $3}')
NOVA_PASS=$(cat openstack_passwords.txt | grep nova_pass | awk '{print $3}')
...
crudini --set /etc/nova/nova.conf database connection mysql://nova:${NOVA_DBPASS}@${controlle}r/nova
